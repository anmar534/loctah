...
import { getProduct, updateProduct, listCategories } from "@/lib/api/admin";
    ...
    const fetchData = async () => {
      try {
        ...
        if (productData.images && productData.images.length > 0) {
          setImages(productData.images.map(img => typeof img === 'string' ? img : img.url));
        } else if (productData.image) {
          setImages([productData.image]);
        }

        if (productData.attributes) {
          const specs: Specification[] = Object.entries(productData.attributes).map(
            ([key, value]) => ({
              key,
              value: String(value),
            })
        ...
        <Card className="p-6">
          <div className="space-y-6">
            ...
            <div>
              ...
              <ImageUpload
                value={images}
                onChange={(value) => setImages(Array.isArray(value) ? value : [value])}
                multiple={true}
                maxFiles={5}
              />
            </div>
